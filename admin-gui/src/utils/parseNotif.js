...
const parseNotif = (data) => {
  ...
  let secondary = (
    <>
      <label style={{ fontWeight: 'bold', marginRight: 4 }}>{data.sender.name}</label>
  ...
  if (data.notifType === notifType.ADD_RM_CL || data.notifType === notifType.DONE_RM_CL) {
    secondary = (
      <>
        <label style={{ fontWeight: 'bold', marginRight: 4 }}>{data.sender.name}</label>
        <label style={{ marginRight: 4 }}>{data.notifType}</label>
        <label style={{ fontWeight: 'bold', marginRight: 4 }}>{data.content.itemName}</label>
        <label style={{ marginRight: 4 }}>for step</label>
        <label style={{ marginRight: 4, fontWeight: 'bold' }}> {data.content.stepName}</label>
